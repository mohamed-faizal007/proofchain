...
### [x] P4-01 ProofChainRegistry.sol + tests
...
Accept (P2-review repo bypass, owned here): `RevisionRepository` must not expose writes that skip the state guards. Override the inherited `update_one` to raise (no caller in `app/`), and make `delete` delete only PENDING revisions (its only callers are the P5-01/P5-02 rollbacks, which delete a revision they just inserted). Tests: `update_one` raises; `delete` on APPROVED/REJECTED/REVOKED is a no-op returning False; the P5-01/P5-02 rollback fault tests stay green.
