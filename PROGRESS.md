...
- P2 phase review (2026-09-20), no HIGH findings. Suites: backend 392 passed (total coverage 95%; `app/deps.py` 0% until P3+ uses it), ruff/mypy clean, contracts 1 passed, frontend 15 passed + lint clean. `-m mongo`/`-m minio` integration tests were not run in this review. MEDIUM:
  ...
  - `RevisionRepository` still inherits unguarded `update_one`/`delete` from `BaseRepository`, bypassing the PENDING guard; protect or remove them on revisions/trees. **Owner: P5-04** (Accept line in TASKS.md). (`set_anchor` now filters on `status == "APPROVED"` as a backstop, FIXED in P2-review; the primary check is the P5-04 service invariant, now in its Accept line.)
- P2 phase review LOW:
...
- P5-03 interpretation choices and limits (review, `ReviewService`):
  ...
  - Wrong state is 409 `REVISION_NOT_PENDING` (existing code in 04), not the generic `CONFLICT` from the plan. The P2-review `update_one`/`delete` bypass on `RevisionRepository` is still open; owner is P5-04 (TASKS.md Accept line).

## Follow-ups (ideas deliberately deferred — do not implement without a task)
- **RevisionRepository write bypass (P2 review, carried through P5-01..P5-03): OWNER P5-04.** Now a P5-04 Accept line, not a deferred idea: `update_one` raises, and `delete` deletes only PENDING revisions. `TreeRepository.delete` stays unguarded (trees have no state; only the rollbacks call it).
