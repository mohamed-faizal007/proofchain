...
- P4 review (2026-09-25, `code-reviewer` on the chain client; no HIGH; reviewer did not run tests). MEDIUM, all in `Web3RegistryClient` / config unless noted:
  ...
  - `_send` puts raw exception text into `AnchorFailedError` (`Contract rejected the transaction: {exc}`), which reaches the API response; use a fixed message and log only the class/custom-error name. Fix in P5-04.
  ...
  - `Web3RegistryClient` has no node-free unit tests (error mapping, receipt status 0, missing event, lock/nonce path, health with wrong chain_id); only `-m chain` covers it. Add stubbed-AsyncWeb3 tests in P5-04.
