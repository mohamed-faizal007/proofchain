...
## Current status
- Phase: P5 in progress (P5-01 to P5-04 done)
- Next task: P5-05 Revoke, list/detail, tree, presigned file, provenance endpoints
- Blockers: none
...
- P5-04 interpretation choices and limits (anchoring, `AnchorService` / `Reconciler`):
  ...
  - **Pointer:** `latest_approved_version_no` is the on-chain version of the pointed-to revision, null while it is anchoring or FAILED (03 note). The reconciler also clears the pointer if a document has no APPROVED revision left (not reachable before P5-05 revoke).
