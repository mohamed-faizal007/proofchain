...
- P2 phase review (2026-09-20), no HIGH findings. Suites: backend 392 passed (total coverage 95%; `app/deps.py` 0% until P3+ uses it), ruff/mypy clean, contracts 1 passed, frontend 15 passed + lint clean. `-m mongo`/`-m minio` integration tests were not run in this review. MEDIUM:
  ...
  - State change and event are two separate Mongo writes (e.g. `set_review` then `append(REVISION_APPROVED)`), not atomic: a crash between them leaves an APPROVED revision with no event. The P5 services must fix the write order or use a transaction, plus a reconcile check and a test. (P5-03: state-then-event with a rollback if the append fails. DONE (P5-04): the startup reconciler appends a missing REVISION_APPROVED/REVISION_REJECTED event after a 60 s grace window, `test_reconciler.py`. DONE (P5-05): REVOKED revisions with no VERSION_REVOKED event, `test_reconciler_revoke.py`.)
  - FIXED (P5-04): `RevisionRepository.update_one` raises `NotImplementedError` and `delete` only deletes PENDING revisions; `set_anchor` was replaced by claim/finish methods that all require `status == "APPROVED"`, and the service raises `REVISION_NOT_APPROVED` first. Tests: `test_revision_anchor_repo.py`; P5-01/P5-02 rollback fault tests unchanged and green.
- P2 phase review LOW:
...
- P4 review (2026-09-25, `code-reviewer` on the chain client; no HIGH; reviewer did not run tests). MEDIUM, all in `Web3RegistryClient` / config unless noted:
  - FIXED (P5-04): unmapped web3 errors. Connectivity (`OSError`, aiohttp, `TimeoutError`, `ProviderConnectionError`, `RequestTimedOut`, `TooManyRequests`) -> `ChainUnavailableError` (retried); `TimeExhausted` -> `AnchorFailedError` (not auto-retried, the tx may still be mined); `ContractLogicError` in `_rpc` and any other `Web3Exception` (RPC, ABI encoding) -> `AnchorFailedError`. Fixed messages, no cause. Tests in `test_web3_error_mapping.py`.
  ...
  - FIXED (P5-04 + follow-up): duplicate anchor after a receipt timeout. Every send re-checks the chain first (client idempotency), a receipt timeout is FAILED without an automatic retry, and `_send` refuses (`AnchorFailedError`, so FAILED) while the anchor account has a pending nonce above its mined one, i.e. while an earlier tx is still in the mempool. Residual: relies on the single-worker assumption (another process using the same key also blocks sends until its tx is mined). See "P5-04" below.
...
- P5-03 interpretation choices and limits (review, `ReviewService`):
  ...
  - A pointer write that fails after the event is logged, and the request still returns 202. Repaired by the P5-04 reconciler (compare-and-set, 60 s grace).
  ...
  - Wrong state is 409 `REVISION_NOT_PENDING` (existing code in 04), not the generic `CONFLICT` from the plan. The P2-review `update_one`/`delete` bypass on `RevisionRepository` was fixed in P5-04.

- P5-04 interpretation choices and limits (anchoring, `AnchorService` / `Reconciler`):
  - **Single-worker assumption (decided, no guard built).** The web3 nonce lock is per process, and a claim is only taken over once it is older than 10 minutes (`attempted_at`), which assumes one API worker. With several uvicorn workers or replicas, two processes could send txs with colliding nonces, and a reconciler on one worker could re-send a claim another worker is still processing only after 10 minutes. **Revisit at P10+ if multiple workers are ever introduced** (options: a leader-elected anchoring worker, or a Mongo lease plus a nonce manager).
  - **Anchor ordering:** revision N is not sent while an older APPROVED revision of the same document is not ANCHORED; it stays queued (`ANCHORING`, `attempted_at` null) and is sent when the predecessor anchors (successor drain in `AnchorService.anchor`). Approval itself is never blocked. A queued revision behind a FAILED predecessor waits for the admin retry of that predecessor (or the next startup reconcile).
  - **Retries:** 3 attempts (1 s, then 4 s) only for `ChainUnavailableError`; contract/node rejection and receipt timeouts go straight to FAILED. `anchor.attempts` counts claims, not chain calls.
  - **Reconciler runs once at startup only** (background task, off under APP_ENV=test unless `create_app(reconcile_on_startup=True)`); there is no periodic run. A FAILED revision is re-sent at every restart.
  - **FAILED with no ANCHOR_FAILED event is deliberately NOT repaired**: the next retry writes its own events.
  - **Pointer:** `latest_approved_version_no` is the on-chain version of the pointed-to revision, null while it is anchoring or FAILED (03 note). The reconciler also clears the pointer if a document has no APPROVED revision left (reachable since P5-05 revoke; `test_pointer_to_a_revoked_revision_is_cleared`).
  - Events written by anchoring have `actor_id: null` (system) and `data.trigger` = approve / retry / reconcile / successor; reconciler-appended events carry `data.reconciled: true` and use the repair time as `at`, so a repaired REVISION_APPROVED can sit after VERSION_ANCHORED in the chain.
  - Retry endpoint on a queued or in-flight anchor returns 409 `CONFLICT` (existing generic code); a stuck claim cannot be force-retried by an admin, only by the startup reconciler after 10 minutes.
  - Untested branches (coverage): document missing during anchoring, "claim lost" after the chain call or before recording a failure, a pointer-step exception inside the reconciler, and a startup-reconcile crash. All only log.

- P5-05 interpretation choices and limits (revoke, reads; `RevocationService` / `QueryService`):
  - **Maker != checker does not apply to revoke (decided).** Revoke is a corrective action on an already anchored fact, not a first-line approval gate, so restricting it to a different approver would add friction without a matching security benefit: the approval it undoes already passed maker-checker, and every revoke is recorded (actor, reason) on-chain and in provenance. Any APPROVER may revoke, including the one who approved (`test_any_approver_may_revoke_including_the_one_who_approved`). ISSUER and ADMIN cannot (403).
  - **`GET /documents?status=X` multi-match is intentional (decided).** A document matches every status any of its revisions has, so v1 APPROVED + v2 PENDING is listed under both `status=APPROVED` and `status=PENDING`, never twice in one list. Pinned by `test_status_filter_matches_document_under_every_status_its_revisions_have` and stated in 04. There is no per-document "status" field; if one is ever wanted (e.g. "current state"), it is a new filter, not a change to this one.
  - Revoke is **synchronous** (200) and requires `anchor.status == ANCHORED` (409 `CONFLICT` + `details.anchor_status` otherwise; not-APPROVED stays 409 `REVISION_NOT_APPROVED` + `details.status`). An APPROVED revision whose anchor FAILED cannot be revoked until it is anchored (retry-anchor first).
  - **Write order: chain -> revision (conditional on APPROVED + ANCHORED) -> VERSION_REVOKED event -> pointer.** Chain errors (503/502) write nothing. If the Mongo write fails after the chain revoke, chain and Mongo disagree (chain revoked, Mongo APPROVED) until the revoke is retried; the retry reads the chain, sends no second tx, and records `revocation.tx_hash = null` + event `already_revoked: true`. That window is NOT repaired automatically (the reconciler does not scan the chain); the P6 RECORD_MISMATCH follow-up (compare on-chain `revoked`) is what would detect it.
  - Event or pointer failure after the state write: 200 anyway, logged (class names and ids only); the reconciler appends VERSION_REVOKED and repairs the pointer (`test_reconciler_revoke.py`).
  - Concurrent revokes are serialized by one in-process `asyncio.Lock` (per event loop): one tx, one event, the loser gets 409. Same single-worker assumption as anchoring (see P5-04).
  - `reason`: required, max 500 chars, and **public** (written on-chain in `VersionRevoked`). Noted in 03/04; the API does not scan it for personal data.
  - Pointer after revoke: newest remaining APPROVED revision (its `version_no`, or null if it is still anchoring), or null/null. Re-submitting the same content after a revoke anchors a fresh version (`test_reanchoring_same_content_after_revoke_is_a_new_version`).
  - Reads: any authenticated user sees every document (04 "any"); there is no per-owner visibility. `GET /documents/{id}` takes the latest approved revision from the revisions, not from the pointer. `q` is regex-escaped. `/revisions/{id}/file` still has the "Presigned URL host" limit (Follow-ups, P8-03/P10-03).
  - Untested defensive branches in `revocation.py` (lines 104-105, 115, 188): document missing, and the revision changed outside the lock between the chain call and the state write. All raise or log.
...
### 2026-09-26 — P5-04 Anchoring service (background + reconcile + retry)
- Done: `AnchorService` (`app/services/anchoring.py`): APPROVED check (409 `REVISION_NOT_APPROVED`), per-document ordering (queue N until older APPROVED revisions are ANCHORED, then drain queued successors), conditional claim with `anchor.attempted_at` (live claims untouched, >10 min = stuck), 3 attempts with 1 s/4 s backoff for `CHAIN_UNAVAILABLE` only, error codes only (`CHAIN_UNAVAILABLE`, `ANCHOR_FAILED`, `CHAIN_NOT_CONFIGURED`, `UNEXPECTED_ERROR`), ANCHORED -> `VERSION_ANCHORED` -> `latest_approved_version_no`; FAILED -> `ANCHOR_FAILED`. Approve schedules it as a BackgroundTask (client read from `app.state`, so no 503). `POST /revisions/{id}/retry-anchor` (ADMIN): FAILED 202, ANCHORED 200, queued/in flight 409 `CONFLICT`. `Reconciler` (`app/services/reconciler.py`), started as a background task by the lifespan: missing review events, ANCHORED-missing-VERSION_ANCHORED, anchor retries, stale pointers (CAS), 60 s grace. Repos: guarded claim/requeue/mark methods replace `set_anchor`; `update_one` raises, `delete` PENDING only; `set_latest_approved` resets the version to null; `repair_pointer`, `set_latest_version_no`, `revision_ids_with_event`. web3 error mapping completed (P4 review). Docs: 03 (pointer semantics, anchor fields, ordering), 04 (`REVISION_NOT_APPROVED`, retry-anchor, background anchoring, null `tx_hash`).
- Tests: `test_anchoring.py` + `test_anchor_ordering.py` (26), `test_reconciler.py` (23), `test_anchor_routes.py` (11), `test_revision_anchor_repo.py` (18), 7 new web3 mapping tests; `-m chain` `test_anchor_real_chain.py` (approve -> ANCHORED on the real Hardhat node + real Mongo, v2 links to v1, retry on ANCHORED is a no-op). 740 passed (coverage 96%), `-m chain` 5 passed, `-m mongo` 14 passed; ruff/format/mypy clean. Mutation checks: disabling ordering fails 4 tests (incl. the v2/v3 bug test), retrying every error fails 4, disabling the successor drain fails 2, dropping the APPROVED check fails 9.
- Decisions: see Known issues "P5-04" (single worker, startup-only reconciler, FAILED-missing-event not repaired, actor null + `trigger`). No ADR (no spec/CANON change; ADR-012 unchanged).
- Issues: stubbed-AsyncWeb3 node-free tests (P4 review) not done, carried to the P5 phase review; P8-06 must handle `tx_hash: null` (Follow-ups).
- Next: P5-05 Revoke, list/detail, tree, presigned file, provenance endpoints (extend the reconciler to REVOKED).
- Follow-up (same day, after review questions): (1) confirmed the successor drain stops at the first non-ANCHORED outcome; `test_failure_mid_drain_leaves_later_revisions_queued` (3-deep queue, middle FAILS, v4 stays queued and unsent, then drains after retrying v3). (2) the two retry-anchor 409s are distinguishable: `CONFLICT` + `details.anchor_status` (queued/in flight; act on the blocker) vs `REVISION_NOT_APPROVED` + `details.status` (never anchorable); `test_retry_409s_are_distinguishable_queued_vs_not_approved`. (3) cheap duplicate-tx mitigation added in `Web3RegistryClient._send` (pending-nonce check), 2 node-free tests; `-m chain` still green.

### 2026-09-26 — P5-05 Revoke, list/detail, tree, presigned file, provenance endpoints
- Done: `POST /revisions/{id}/revoke` (APPROVER, `{reason}`, synchronous 200). `RevocationService` (`app/services/revocation.py`): 404 -> 409 `REVISION_NOT_APPROVED` -> 409 `CONFLICT` (not ANCHORED) -> 503 if no chain; then chain first (`get_version`, skip the tx if already revoked, else `revoke_version`) -> `RevisionRepository.mark_revoked` (APPROVED + ANCHORED only) -> VERSION_REVOKED event -> pointer compare-and-set to the newest remaining APPROVED revision. New `Revision.revocation {by, at, reason, tx_hash, block_number}`. Read routes via `QueryService` (`app/services/queries.py`): `GET /documents` (page/page_size<=100, literal case-insensitive `q`, `doc_type`, `status`), `GET /documents/{id}` (`{document, latest_approved_revision}`), `/documents/{id}/revisions`, `/documents/{id}/provenance` (`chain_valid` + events), `GET /revisions/{id}`, `/tree`, `/file` (`{url, expires_in}`, pinned S3 version). Reconciler: REVOKED revisions with no VERSION_REVOKED event get it (60 s grace), and REVOKED revisions are also checked for a missing REVISION_APPROVED. Docs: 03 (revocation field, pointer after revoke, state machine), 04 (revoke errors, read-route shapes, status multi-match).
- Tests: `test_revoke.py` (18), `test_revoke_faults.py` (7: chain 503/502 writes nothing, version missing on-chain, DB failure after chain revoke finished by a retry with no second tx, event and pointer failures logged without exception text, concurrent revokes -> one tx/one event/409), `test_reconciler_revoke.py` (6: missing and present VERSION_REVOKED, grace window, missing REVISION_APPROVED on a REVOKED revision, never re-anchored, pointer to a revoked revision cleared), `test_document_queries.py` (21 incl. the status multi-match test), `test_revision_queries.py` (11), `test_revision_revoke_repo.py` (7); `-m chain` `test_revoke_on_real_chain`. Full suite 814 passed (coverage 96%; `revocation.py` 96%, `queries.py` 100%), `-m chain` 6 passed on the running Hardhat node, `-m mongo` 15 passed; ruff/format/mypy clean. Mutation checks (6): removing the revoke lock, the already-revoked chain check, the ANCHORED requirement, the `q` regex escape, the reconciler revoke step or the pointer repair each fails at least one test. The concurrency test first MISSED the lock removal (its barrier released both callers before they read the chain, so the loser took the already-revoked path); fixed to read before the barrier and re-verified both ways.
