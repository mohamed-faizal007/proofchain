...
- P2 phase review (2026-09-20), no HIGH findings. Suites: backend 392 passed (total coverage 95%; `app/deps.py` 0% until P3+ uses it), ruff/mypy clean, contracts 1 passed, frontend 15 passed + lint clean. `-m mongo`/`-m minio` integration tests were not run in this review. MEDIUM:
  - **S3 timeouts vs public /health (fix before P3/P5):** `S3Storage.from_settings` sets no `connect_timeout`/`read_timeout` (botocore default 60s, 3 attempts) and `asyncio.wait_for` in `check_health` cancels only the await, not the anyio worker thread. With S3 unreachable, ~40 concurrent unauthenticated `/health` calls can exhaust the thread pool and stall every other `to_thread` call. Set `Config(connect_timeout=2, read_timeout=10, retries={"max_attempts": 2})` and consider a short TTL cache for the S3 probe.
  ...
  - `RevisionRepository` inherits unguarded `update_one`/`delete` from `BaseRepository`, bypassing the PENDING guard; `set_anchor` does not require `status == "APPROVED"`, so "only APPROVED revisions are anchored" is not enforced at the persistence layer. Filter `set_anchor` on APPROVED; protect or remove `update_one`/`delete` on revisions/trees.
- P2 phase review LOW:
  - `set_anchor` returns `modified_count > 0`, so a retry with an identical anchor reads as "not found"; use `matched_count`. Document counters (`revision_count`, `latest_approved_*`, `updated_at`) have no atomic helper yet (P5).
