...
- P4 review (2026-09-25, `code-reviewer` on the chain client; no HIGH; reviewer did not run tests). MEDIUM, all in `Web3RegistryClient` / config unless noted:
  ...
  - Duplicate anchor after a receipt timeout, PARTLY FIXED (P5-04): every send re-checks the chain first (client idempotency), and a receipt timeout is FAILED without an automatic retry. Remaining: an admin retry (or restart reconcile) while that tx is still in the mempool sends a second tx. Single-worker assumption: see "P5-04" below.
